...
const QuestionMap = require("../models/QuestionMap");
...
router.post(
  ...
  async (req, res) => {
    try {
      const { class_number,assessment_task,question_number,clo, weightage} = req.body;
      ...
      const newQuestionMap= new QuestionMap({
        class_number,assessment_task,question_number,clo, weightage,
        user: req.user.id,
      });
